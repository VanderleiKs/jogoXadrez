...
public class ChessMatch {
	...
	public ChessPiece performChessMove(ChessPosition sourcePosition, ChessPosition targetPosition) {
		...
		if (testCheck(currentPlayer)) {
			...
			throw new ChessException("Error: You cannot put yourself in check!");
		}
		...
		if(movePiece instanceof Pawn && (target.getRow() == source.getRow() + 2 || target.getRow() == source.getRow() - 2)) {
			endPassantVulnerable = movePiece;
		}
		else {
			endPassantVulnerable = null;
		}
	...
	public ChessPiece replacePromotedPiece(String type) {
		...
		if(!type.equals("B") && !type.equals("N") && !type.equals("Q") && !type.equals("R")) {
			throw new InvalidParameterException("Invalid for promoted, must be:B/N/Q/R");
		}
	...
	private Piece makeMove(Position source, Position target) {
		...
		if (p instanceof King && target.getColumn() == source.getColumn() + 2) {
			...
			ChessPiece t = (ChessPiece) board.removePiece(sourceT);
	...
	private void undoMove(Position source, Position target, Piece capturedPiece) {
		...
		if (p instanceof King && target.getColumn() == source.getColumn() + 2) {
			Position sourceT = new Position(source.getRow(), source.getColumn() + 3);
			Position targetT = new Position(source.getRow(), source.getColumn() + 1);
			ChessPiece t = (ChessPiece) board.removePiece(targetT);
			board.placePiece(t, sourceT);
			t.decreaseMoveCount();
		}
		...
		if(p instanceof Pawn) {
			if(source.getColumn() != target.getColumn() && capturedPiece == endPassantVulnerable) {
				Position pawnPosition;
				if(p.getColor() == Color.WHITE) {
				...
				ChessPiece sourcePassant = (ChessPiece)board.removePiece(target);
				board.placePiece(sourcePassant, pawnPosition);
			}
		}
	}
	...
	private ChessPiece king(Color color) {
		...
		throw new IllegalStateException("Error: Not exists king!");
	}
